(function (window, document, undefined) 
{
    ...
    (function () 
    {
        ...
        var table = document.createElement( 'table' ),
			...
			var filterData = function( items, keysFilter )
			{
				...
				*/
				...
				{
					this.data.full = args.data;
					
					if( isModeResponsive() )
					{
						args.data = changeAsTwoColumns( args.data, ( args.labelResponsive || this.params.labelResponsive ) );
						this.data.responsive = args.data;
					}

					changeClassModeTable( this, getModeTable() );
					__updateData.call( this, args );				
				},
        ...
        {
        	return isModeResponsive() ? ModeTable.RESPONSIVE : ModeTable.FULL;
        },

        changeClassModeTable = function( table, mode )
        {
   		...
   		onChangeModeTable = function( table, callback )
   		{
   			window.addEventListener( 'resize', function( e )
   			{
	   			...
	   			if( isModeResponsive() && (/tblFull/).test( attrs[ 'class' ] ) )
	   			{
	   				...
	   				//changeClassModeTable( table, ModeTable.RESPONSIVE );
	   			}
	   			else if( isModeTableFull() && (/tblResponsive/).test( attrs[ 'class' ] ) )
	   			{
	   				callback( ModeTable.FULL );
	   				//changeClassModeTable( table, ModeTable.FULL );
	   			}
   			},
   			false );
   		},

   		changeDataModeTable = function( table, mode )
   		{
    		...
    		{
    			table.data.full = table.params.data;
 				if( table.data.responsive === undefined )
    				table.data.responsive = changeAsTwoColumns( table.params.data, params.labelResponsive );
    			data = table.data.responsive;
    		}
    		else
    		{
    			if( table.data.full === undefined )
    				table.data.full = table.params.data;
    			data = table.data.full;
    		}

    		changeClassModeTable( table, mode );

    		__updateData.call( table, {
    			'data' : data
    		});
   		},
        ...
        {
        	var mode = isModeResponsive() ? ModeTable.RESPONSIVE : ModeTable.FULL;

        	//changeClassModeTable( table, mode );
        	changeDataModeTable( table, mode );
