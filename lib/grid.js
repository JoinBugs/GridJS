(function (window, document, undefined) 
{
    ...
    (function () 
    {
        ...
        var table = document.createElement( 'table' ),
			...
			var filterData = function( items, keysFilter )
			{
				...
				*/
				...
				updateData = function( data, columns, columnsName )
				{
        ...
        changeModeTable = function( table )
        {
        	var params     = table.params,
        		paramsUser = table.params.paramsUser,
        		classes    = params.attrs['class'] || '';

				classes = classes.replace( /\s|tblFull|tblResponsive/g, '' );

        		// mode responsive
        		if( window.innerWidth < 768 && (/tblFull/).test( params.attrs[ 'class' ] ) )
        		{
        			table.updateData( changeAsTwoColumns( paramsUser.data,
        												  params.labelResponsive ) );
        			classes += ' tblResponsive';
        			params.attrs['class'] = classes;
        			setAttrs( table, params.attrs );
        		}
        		// mode full table
        		else if ( window.innerWidth > 767 && (/tblResponsive/).test( params.attrs[ 'class' ] ) )
        		{
        			classes += ' tblFull';
        			table.updateData( paramsUser.data );
        			params.attrs['class'] = classes;
        			setAttrs( table, params.attrs );
        		}
        },
        ...
        {
        	//check params and add properties if this ones is not in object params the otherwise its not anything
        	checkParams( params );
