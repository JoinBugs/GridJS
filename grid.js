(function (window, document, undefined) 
{
    ...
    (function () 
    {
				...
				updateData = function( data, columns )
				{
					...
					/*if( columns.length < lastColumns.length )
						for( var i = 0, l = this.rows.length; i < l; i++ )
							for( var j = 0; j <= lastColumns.length - columns.length; j++ )
								this.rows[ i ].cells[ this.rows[0].cells.length - 1 ].remove();*/
		...
		changeAsOneColumn = function( data, label )
        {
            var newData = [];
        ...
        {
        	checkParams( params );
        	window.changeAsOneColumn = changeAsOneColumn;
        	if( params.hasOwnProperty( 'responsive' ) )
        	{
			    if( window.innerWidth < 768 )
			    {
	        		params.data = changeAsOneColumn( params.data, params.responsive );
	        		params.columnsName = [ params.responsive ];
			    }
        	}
